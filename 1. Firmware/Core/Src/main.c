...
int weight = 0;
char strWeight[10];
int Static_peak_delay = 10; // after this, kinetic sample will start calculating
int idx = 0;
int COF_Time = 0;
int Data_Weight[4000];
...
//		FLASH_WritePage(FLASH_CCW_start_addr,FLASH_CCW_end_addr,CCW_limit);
	...
	else if (GPIO_Pin == GPIO_PIN_6 && (currentMillis - previousMillis > 150)) // Journey switch
	{ 
...
//		FLASH_WritePage(FLASH_CCW_start_addr,FLASH_CCW_end_addr,button_E);
	...
	else if (GPIO_Pin == GPIO_PIN_7 &&  (currentMillis - previousMillis > 150))// Journey switch 2
	{
...
void Acceleration(int Target_Speed)
{
	...
	for (int i = 0; i < 200 ; i++ )
	{
		...
		{
		Accel_Speed = Accel_Speed + (Target_Speed/200);
...
void Deceleration(int Target_Speed)
{
	...
	for (int i = 0; i < 150 ; i++ )
	{
		if (Accel_Speed > 0)
		{
		Accel_Speed = Accel_Speed - (Target_Speed/150);
		Cal_PWM_Period(Accel_Speed);
		Run_Coating();
		HAL_Delay(1);
		}
...
//											Stable_Weight_Val = 0; // if max weight value changed --> stable array will be reseted
										}
//										else if (Max_Weight[Max_Weight_idx] >= Data_Weight[i] && (idx - Max_Weight_Index[Max_Weight_idx]) == 100)
//										{
//											Max_Weight_Index[Max_Weight_idx] = Max_Weight_Index[Max_Weight_idx];
//											Max_Weight[Max_Weight_idx] = Max_Weight[Max_Weight_idx];
//											Max_Weight_idx = Max_Weight_idx + 1;
//											
//										}
										else
										{
								...
								for(int j = Max_Weight_Index[0] + Static_peak_delay; j < idx; j++)
									...
									if (idx - Max_Weight_Index[Max_Weight_idx] == 20 && Max_Weight[Max_Weight_idx] > 500)
									{
										Max_Weight_idx ++;
									}
									idx ++;
	 ...
	 //end
							...
							std_deviation = (sqrt(s1/(idx - (Max_Weight_Index[0]+ Static_peak_delay) - 1)))/100; 							//Standard deviation
							if(CW_limit) 
							{
								CW_COF = 1;
								HAL_TIM_PWM_Stop(&htim3,TIM_CHANNEL_3);
